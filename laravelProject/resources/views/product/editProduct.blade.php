    ...
    <div class="grid md:grid-cols-[8%_1fr_8%] max-[769px]:grid-cols-[2%_1fr_2%] h-screen bg-center bg-cover bg-no-repeat"
        ...
        <div class="col-start-2 flex mt-3 flex-wrap flex-row justify-center">
            <div
                ...
                <form action="{{route('product.update', ['product' => $product])}}" method="POST" class="flex justify-around flex-col h-5/6 w-2/5">
                    @method('PUT')
                    @csrf
                    <h1 class="text-lg font-semibold text-center">bewerk hier je product</h1>

                    <div class="">
                        <label for="" class="">product naam:</label>
                        <input type="text"
                            class="rounded-xl bg-transparent border-[#272727] placeholder:text-gray-500 placeholder:font-medium"
                            placeholder="hoesje" name="product_name" required autocomplete="off" value="{{$product->product_name}}">
                    </div>
                    <div>
                        ...
                        <input type="text"
                            ...
                            placeholder="samsung" name="product_brand" required autocomplete="off" value="{{$product->product_brand}}">
                    </div>
                    <div>
                        <label for="" class="">product prijs:</label>
                        <input type="text"
                            class="rounded-xl bg-transparent border-[#272727] placeholder:text-gray-500"
                            placeholder="€9,99" name="product_price" title="gebruik het euroteken" required autocomplete="off" value="{{$product->product_price}}">
                    </div>
                    <div>
                        <label for="" class="">product voorraad:</label>
                        <input type="text"
                            class="rounded-xl bg-transparent border-[#272727] placeholder:text-gray-500"
                            name="product_inStock" autocomplete="off" required value="{{$product->product_inStock}}">
                    </div>
                    <input type="file" name="product_img" id="" class="">
                    <input type="submit" value="bijwerken" class="border rounded-2xl w-2/3">
                </form>
            </div>
        </div>
    </div>
